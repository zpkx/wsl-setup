...
echo ""
echo "后续手动步骤:"
echo "  1. 重启 WSL 终端或执行: exec zsh"
echo "  2. 运行 sheldon lock 生成插件锁定文件"
echo "  3. (可选) 运行启动性能测试:"
echo "     for i in \$(seq 1 5); do"
echo '       /usr/bin/time -f "%e seconds" zsh -i -c exit 2>&1'
echo "     done"
echo "  4. 如果 \`brew shellenv\` 报错，忽略或安装 Homebrew"
echo "  5. 手动在 .zshrc 中添加 GITHUB_TOKEN 等敏感环境变量"
echo ""
